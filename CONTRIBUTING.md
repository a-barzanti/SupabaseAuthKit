...
The portable package lives in `skills/supabase-authkit`; the root app demonstrates it. Make authorization changes in the skill's SQL/tests first, update its matrix and integration guidance, then run `bun run skill:sync` and `bun run skill:check`. Do not edit deployed migration history; release upgrades need new forward migrations. The initial unreleased 0.2.0 assets are mirrored into `supabase/migrations` for the reference app.
